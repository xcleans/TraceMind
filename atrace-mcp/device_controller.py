"""DeviceController — composition facade over atrace-device + atrace-capture.

Delegates ADB / HTTP / atrace-tool interactions to ``atrace-device``,
routes config-driven capture through ``atrace-capture``, and adds
profiling orchestration (simpleperf, heapprofd) that needs
tool_provisioner and therefore lives in the MCP layer.
...
from atrace_device.engine_cli import EngineResult  # noqa: E402
from atrace_capture import CaptureRouter  # noqa: E402
from atrace_capture.config.schema import CaptureConfig  # noqa: E402
from atrace_capture.config.registry import ConfigRegistry  # noqa: E402
...
class DeviceController:
    ...
    def _try_gecko(
        ...
        if tool_provisioner.run_gecko_profile_generator(toolkit, local_perf, gecko_path):
