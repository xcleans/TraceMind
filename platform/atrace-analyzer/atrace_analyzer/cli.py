#!/usr/bin/env python3
"""Headless Perfetto trace analysis — same ``TraceAnalyzer`` as MCP, no AI / no MCP.

Install ``atrace-analyzer`` and run:
...
Or from repo:
  ...
  cd atrace-analyzer && uv run python -m atrace_analyzer.cli overview /path/to/trace.perfetto
"""
...
from atrace_analyzer import TraceAnalyzer
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Analyze Perfetto traces with TraceAnalyzer (headless JSON). "
            "Same engine as atrace-mcp; no MCP / no AI."
        ),
    )
