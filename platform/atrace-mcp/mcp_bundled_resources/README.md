...
```bash
./dev-setup.sh uv     # 或 ./dev-setup.sh
./gradlew deployMcp   # 生成 atrace-tool JAR（若尚未）
```

完成后 `**import atrace_analyzer**`、`**import atrace_capture**` 可用，无需手写 `PYTHONPATH`。

**仅在本目录开发**（依赖 `../atrace-capture` 等 sibling 时）：
...
若存在仓库内 `**platform/_monorepo.py**`，`run_mcp.py` / `server.py` 会 `**bootstrap()**` 解析 sibling 包路径。

**安装后校验**

```bash
cd atrace-mcp
uv run python -c "from trace_analyzer import TraceAnalyzer; from atrace_capture.device_controller import DeviceController; print('ok')"
```
...
./platform/atrace-mcp/scripts/build_release.sh 0.2.0     # 指定 MCP 版本（zip / wheel 名）
```

概要：`**./gradlew deployMcp**` → `**atrace-capture` uv build** → `**atrace-mcp` uv build** → `**dist/*.whl`** + `**dist/atrace-mcp-v<MCP>.zip**`。
...
**仅 pip wheel**（须自行解决 `atrace-analyzer` 导入）：

```bash
pip install dist/atrace_capture-*.whl dist/atrace_mcp-*.whl
```
...
### 4.4 接入 Cursor（`mcp.json`）

1. 编辑项目 `**.cursor/mcp.json**` 或全局 `**~/.cursor/mcp.json**`。
2. `**uv run --directory**` 指向 `**platform/atrace-mcp**`（开发：`…/TraceMind/platform/atrace-mcp`；离线 bundle：`…/bundle/platform/atrace-mcp`，且 bundle 根含上表各目录与 `**platform/_monorepo.py**`）。
  ...
  "mcpServers": {
    "atrace": {
      ...
      "args": [
        "run",
        "--directory",
        "/ABS/PATH/TraceMind/platform/atrace-mcp",
        "python",
        "run_mcp.py"
      ]
    }
...
工作区根为 TraceMind 时可用 `**${workspaceFolder}/atrace-mcp**` 作为 `--directory`。
