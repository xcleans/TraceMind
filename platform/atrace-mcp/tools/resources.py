"""MCP Resources — Perfetto scenario ``.txtpb`` (from ``atrace-capture``) + SQL reference."""
...
def _perfetto_scenario_configs_dir() -> Path | None:
    """Directory of ``*.txtpb`` for MCP ``atrace://configs/*`` resources."""
    override = (
        os.environ.get("ATRACE_PERFETTO_CONFIGS", "").strip()
        or os.environ.get("ATRACE_DOCS_CONFIGS", "").strip()
    )
    if override:
        ...
        if p.is_dir():
            return p
    try:
        from atrace_capture.config.perfetto_paths import bundled_perfetto_configs_dir

        d = bundled_perfetto_configs_dir()
        if d.is_dir():
            return d
    except ImportError:
        pass
...
def _docs_configs_dir() -> Path | None:
    """Backward-compatible alias for :func:`_perfetto_scenario_configs_dir`."""
    return _perfetto_scenario_configs_dir()
...
def _read_docs_config_file(filename: str) -> str:
    ...
    if root is None:
        return (
            "# Perfetto scenario configs directory not found\n\n"
            "Install `atrace-capture` or set `ATRACE_PERFETTO_CONFIGS` / `ATRACE_DOCS_CONFIGS` "
            "to a folder containing the `.txtpb` files.\n"
        )
    path = root / filename
    if not path.is_file():
...
def register_resources(mcp) -> None:
    ...
    @mcp.resource("atrace://configs/readme", title="Perfetto configs README", mime_type="text/markdown")
